...
def _use_gated_delta_loop(query: torch.Tensor) -> bool:
    if os.environ.get("QEFF_QWEN3_5_DISABLE_GATED_DELTA_LOOP", "0") == "1":
        return False
    return os.environ.get("QEFF_QWEN3_5_ENABLE_GATED_DELTA_LOOP", "1") != "0" and query.shape[1] == 1
...
def qeff_apply_interleaved_mrope(freqs, mrope_section):
    ...
    args:
        freqs: (3, bs, seq_len, head_dim)
        mrope_section: (3,)
    returns:
        freqs_t: (bs, seq_len, head_dim)
    """

    feature_indices = torch.arange(freqs.shape[-1], device=freqs.device)
    source_dim = torch.zeros_like(feature_indices)
    half_shape = freqs.shape[-1] // 2

    for dim, offset in enumerate((1, 2), start=1):
        length = mrope_section[dim] * 3
        first_half_mask = (feature_indices >= offset) & (feature_indices < length)
        first_half_mask = first_half_mask & ((feature_indices - offset) % 3 == 0)
        second_offset = offset + half_shape
        second_length = length + half_shape
        second_half_mask = (feature_indices >= second_offset) & (feature_indices < second_length)
        second_half_mask = second_half_mask & ((feature_indices - second_offset) % 3 == 0)
        source_dim = torch.where(first_half_mask | second_half_mask, torch.full_like(source_dim, dim), source_dim)

    gather_index = source_dim.view(1, 1, -1, 1).expand(freqs.shape[1], freqs.shape[2], -1, 1)
    return torch.gather(freqs.permute(1, 2, 3, 0), dim=-1, index=gather_index).squeeze(-1)
...
def qeff_prepare_mrope_cos_sin(cos, sin, position_ids, mrope_section, dtype=None):
    ...
    mrope_dim, batch_size = safe_position_ids.shape[:2]
    cos = cos.index_select(0, flat_pos).reshape(mrope_dim, batch_size, -1, cos.shape[-1])
    sin = sin.index_select(0, flat_pos).reshape(mrope_dim, batch_size, -1, sin.shape[-1])
...
class QEffQwen3_5GatedDeltaNet(Qwen3_5GatedDeltaNet):
    ...
    def _torch_chunk_gated_delta_rule_loop(
        self,
        query,
        key,
        value,
        ...
        query, key, value, g, beta = [tensor.movedim(2, 0).contiguous() for tensor in (query, key, value, g, beta)]
        # Use canonical contiguous metadata for the loop carry. Dynamo while_loop
        # requires body outputs and carried inputs to have identical strides.
        core_attn_out = torch.zeros(
            value.shape, dtype=value.dtype, device=value.device
        ).contiguous()
        loop_index = torch.zeros((), dtype=torch.int64, device=value.device)

        def loop_cond(index, recurrent_state, output):
            return index < sequence_length

        def loop_body(index, recurrent_state, output):
            active = index < sequence_length
            last_index = torch.full((), sequence_length - 1, dtype=torch.int64, device=value.device)
            token_index = torch.minimum(index, last_index).reshape(1)
            query_t = query.index_select(0, token_index).squeeze(0)
            key_t = key.index_select(0, token_index).squeeze(0)
            value_t = value.index_select(0, token_index).squeeze(0)
            g_t = g.index_select(0, token_index).squeeze(0)
            beta_t = beta.index_select(0, token_index).squeeze(0)

            next_recurrent_state = recurrent_state * g_t.exp().unsqueeze(-1).unsqueeze(-1)
            kv_mem = torch.einsum("bhkv,bhk->bhv", next_recurrent_state, key_t)
            delta = (value_t - kv_mem) * beta_t.unsqueeze(-1)
            next_recurrent_state = next_recurrent_state + key_t.unsqueeze(-1) * delta.unsqueeze(-2)
            output_t = torch.einsum("bhkv,bhk->bhv", next_recurrent_state, query_t)

            recurrent_state = torch.where(active, next_recurrent_state, recurrent_state)
            output_mask = (torch.arange(output.shape[0], device=output.device) == index).reshape(-1, 1, 1, 1)
            output_mask = output_mask & active
            output = torch.where(output_mask, output_t.unsqueeze(0), output).contiguous()
            return index + 1, recurrent_state, output

        _, last_recurrent_state, core_attn_out = while_loop(
            loop_cond,
            loop_body,
            (loop_index, last_recurrent_state, core_attn_out),
        )

        if not output_final_state:
            last_recurrent_state = None
        core_attn_out = core_attn_out.movedim(0, 1).contiguous().to(initial_dtype)
    ...
    def forward(
        ...
        # ── Recurrent State ───────────────────────────────────
        if cache_params is not None:
            if seq_len == 1 and _is_dynamo_compiling() and _use_gated_delta_loop(query):
                # Decode-only path for disaggregated Dynamo exports. Prefill
                # continues to use the chunked rule, keeping the two
                # specializations from sharing incompatible loop shapes.
                core_attn_out, last_recurrent_state = self._torch_chunk_gated_delta_rule_loop(
                    query=query,
                    key=key,
                    value=value,
                    g=g,
        ...
        #
        # ── Output ────────────────────────────────────────────
        # The Dynamo while-loop can carry an output buffer specialized to the
        # prefill sequence length. During decode, `z` is only one token long;
        # align the loop result with `z` before flattening for gated RMSNorm.
        core_attn_out = core_attn_out[:, : z.shape[1]]
...
class QEffQwen3_5DecoderLayer(Qwen3_5DecoderLayer):
    ...
    def supports_dynamo_nested_subfunctions(self):
        return False
...
class QEffQwen3_5ForConditionalGeneration(Qwen3_5ForConditionalGeneration):
    ...
    def get_dummy_inputs(
        ...
        **kwargs,
        ...
        cache_seq_len = constants.INTERN_CTX_LEN if prefill_seq_len == 1 else prefill_seq_len
