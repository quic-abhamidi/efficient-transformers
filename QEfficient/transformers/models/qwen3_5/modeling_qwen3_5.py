...
def _use_gated_delta_loop(query: torch.Tensor) -> bool:
    if os.environ.get("QEFF_QWEN3_5_DISABLE_GATED_DELTA_LOOP", "0") == "1":
        return False
    return os.environ.get("QEFF_QWEN3_5_ENABLE_GATED_DELTA_LOOP", "0") == "1"
...
class QEffQwen3_5GatedDeltaNet(Qwen3_5GatedDeltaNet):
    ...
    def _torch_chunk_gated_delta_rule_loop(
        ...
        core_attn_out = torch.zeros(
            sequence_length, batch_size, num_heads, v_head_dim, dtype=value.dtype, device=value.device
        )
    ...
    def torch_chunk_gated_delta_rule_qeff(
        ...
        if _is_dynamo_compiling() and _use_gated_delta_loop(query):
            return self._torch_chunk_gated_delta_rule_loop(
                query=query,
                key=key,
                value=value,
                g=g,
